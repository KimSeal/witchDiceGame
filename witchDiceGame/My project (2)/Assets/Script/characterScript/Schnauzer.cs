...
public class Schnauzer : Character
{

    public Schnauzer(int curState, Destiny destiny) : base(curState, destiny)
    {
    ...
    public override List<TakeSkillPacket> doSkill(SendSkillPacket sendSkillPacket)
    {
        ...
        if (this.skillIdx[sendSkillPacket.useSkillIdx] == 0) //와와 기본 스킬
        {
            ...
            if(sendSkillPacket.diceNum[0] == 0) packets.Add(new TakeSkillPacket(sendSkillPacket.useCharacterIdx + 4, 0, 1, -999));
            else packets.Add(new TakeSkillPacket(sendSkillPacket.useCharacterIdx + 4, 0, Random.Range(1,sendSkillPacket.diceNum[0]), -999)); //스킬을 사용한 대상에게 / 0변화 / 상태변화를 현재 주사위 값으로 / 스킬은 버프류
        }
