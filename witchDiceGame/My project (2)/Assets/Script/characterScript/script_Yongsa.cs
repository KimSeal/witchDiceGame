    ...
    public override List<TakeSkillPacket> doSkill(SendSkillPacket sendSkillPacket)
    {
        ...
        if (this.destiny.getDestinyIdx() == 0)
        {
            ...
            if (sendSkillPacket.useSkillIdx == 1) //용사 특수 스킬
            {
                ...
                packets.Add(new TakeSkillPacket(sendSkillPacket.targetIdx[0], 1000+ sendSkillPacket.diceNum[0] * sendSkillPacket.diceNum[1] + this.getPhyAtk(), 0)); //선택한 1개의 대상에게 / 대상이 사용한 주사위 값을 기반으로 Damage / 상태변화 없음
            }
        }
        return packets;
    }
